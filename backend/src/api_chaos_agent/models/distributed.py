...
class WorkerCapabilities(BaseModel):
    ...
    supported_protocols: list[str] = Field(
        default_factory=lambda: ["rest"], description="Supported API protocols"
    )
    region: str = Field(
        default="default", description="Geographic region for region-aware scheduling"
    )
    labels: dict[str, str] = Field(
        default_factory=dict, description="Custom labels for filtering and routing"
    )
...
class Worker(BaseModel):
    ...
    last_heartbeat: datetime = Field(
        default_factory=datetime.now, description="Last heartbeat timestamp"
    )
...
class DistributedTask(BaseModel):
    ...
    scenario_ids: list[str] = Field(
        default_factory=list, description="Scenario identifiers in this task"
    )
    status: str = Field(
        default="pending", description="Task status: pending, running, completed, failed"
    )
...
class DistributedExecutionPlan(BaseModel):
    ...
    strategy: str = Field(
        default="round_robin",
        description="Distribution strategy: round_robin, least_loaded, region_aware",
    )
