...
class PipelineConfig(BaseModel):
    ...
    scenario_types: list[str] = Field(
        default_factory=lambda: ["latency", "error_status"], description="Scenario types to run"
    )
    fail_on_severity: str = Field(
        default="high", description="Fail pipeline if finding >= this severity"
    )
...
class PipelineRun(BaseModel):
    ...
    commit_sha: str | None = Field(
        default=None, description="Git commit SHA that triggered the run"
    )
