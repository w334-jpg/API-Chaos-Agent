...
class FaultPluginManifest(BaseModel):
    ...
    config_schema: dict[str, Any] = Field(
        default_factory=dict, description="JSON Schema for plugin config validation"
    )
...
class FaultPlugin(BaseModel):
    ...
    loaded_at: datetime = Field(
        default_factory=datetime.now, description="Timestamp when plugin was loaded"
    )
    error_message: str | None = Field(default=None, description="Error message if status is ERROR")
    execution_count: int = Field(
        default=0, description="Number of times the plugin has been executed"
    )
...
class FaultPluginExecution(BaseModel):
    ...
    input_config: dict[str, Any] = Field(
        default_factory=dict, description="Configuration passed to the plugin"
    )
