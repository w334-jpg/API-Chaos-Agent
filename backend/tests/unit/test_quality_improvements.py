...
class TestPluginRegistry:
    def test_subscribe_and_emit(self):
        from api_chaos_agent.services.plugin_registry import PluginRegistry, PluginEventType
    ...
    def test_unsubscribe(self):
        ...
        registry = PluginRegistry()
        received = []
        handler = lambda et, name, payload: received.append((et, name))
        ...
        spec = {
            "paths": {
                "/login": {
                    "post": {
                        "parameters": [
                            {"name": "password", "in": "query", "schema": {"type": "string", "default": "secret123"}}
                        ]
                    }
                }
            }
...
class TestExceptionHierarchy:
    ...
    def test_schema_parse_error_inherits(self):
        from api_chaos_agent.core.exceptions import SchemaParseError, ChaosAgentError
        err = SchemaParseError(detail="invalid spec")
        assert isinstance(err, ChaosAgentError)

    def test_execution_error_inherits(self):
        from api_chaos_agent.core.exceptions import ExecutionError, ChaosAgentError
        err = ExecutionError(detail="timeout")
        assert isinstance(err, ChaosAgentError)

    def test_plugin_error_inherits(self):
        from api_chaos_agent.core.exceptions import PluginError, ChaosAgentError
